...
class LanguageTestCase(unittest.TestCase):
    ...
    def setUp(self):
        for var in ('x', 'y'):
            if var in list(robjects.globalenv.keys()):
...
"""
        res = lg.eval(code)
        self.assertTrue('x' in list(robjects.globalenv.keys()))
        self.assertEqual(3, robjects.globalenv['x'][0])
        self.assertTrue('y' in list(robjects.globalenv.keys()))
...
"""
        ...
        self.assertTrue('x' in list(env.keys()))
        self.assertEqual(3, env['x'][0])
        self.assertTrue('y' in list(env.keys()))
